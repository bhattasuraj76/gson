 ...
 * configuration
 * options other than the default. For {@link Gson} with default configuration,
 * it is simpler to
 * use {@code new Gson()}. {@code GsonBuilder} is best used by creating it, and
 * then invoking its
...
public final class GsonBuilder {
   ...
   * by first
   * invoking various configuration methods to set desired options, and finally
   * calling
   * {@link #create()}.
   */
  public GsonBuilder() {
   ...
   * on the
   * annotation types {@link Since} and {@link Until}. It allows including or
   * excluding fields
   * and classes based on the specified version. See the documentation of these
   * annotation
   ...
   * which
   * excludes these fields was
   * {@linkplain #setExclusionStrategies(ExclusionStrategy...) registered with
   * this builder}.
   *
   ...
   *         "Builder" pattern
   */
  public GsonBuilder excludeFields(int... modifiers) {
   ...
   * and deserialization. This is a convenience method which behaves as if an
   * {@link ExclusionStrategy}
   * which excludes inner classes was
   * {@linkplain #setExclusionStrategies(ExclusionStrategy...) registered with
   * this builder}.
   * This means inner classes will be serialized as JSON {@code null}, and will be
   * deserialized as
   * Java {@code null} with their JSON data being ignored. And fields with an
   * inner class as type will
   * be ignored during serialization and deserialization.
   *
   * <p>
   * By default Gson serializes and deserializes inner classes, but ignores
   * references to the
   * enclosing instance. Deserialization might not be possible at all when
   * {@link #disableJdkUnsafe()}
   * is used (and no custom {@link InstanceCreator} is registered), or it can lead
   * to unexpected
   ...
   * field and cache the result. It is not guaranteed that the strategy will be
   * used
   ...
   * disjunction rule.
   * This means that if one of the {@code strategies} suggests that a field (or
   * class) should be
   * skipped then that field (or object) is skipped during
   * serialization/deserialization.
   * The strategies are added to the existing strategies (if any); the existing
   * strategies
   * are not replaced.
   *
   * <p>
   * Fields are excluded for serialization and deserialization when
   * {@link ExclusionStrategy#shouldSkipField(FieldAttributes) shouldSkipField}
   * returns {@code true},
   * or when {@link ExclusionStrategy#shouldSkipClass(Class) shouldSkipClass}
   * returns {@code true}
   * for the field type. Gson behaves as if the field did not exist; its value is
   * not serialized
   * and on deserialization if a JSON member with this name exists it is skipped
   * by default.<br>
   * When objects of an excluded type (as determined by
   * {@link ExclusionStrategy#shouldSkipClass(Class) shouldSkipClass}) are
   * serialized a
   * JSON null is written to output, and when deserialized the JSON value is
   * skipped and
   ...
   * field or
   * class and cache the result. It is not guaranteed that the strategy will be
   * used again
   ...
   * serialization.
   * If this method is invoked numerous times with different exclusion strategy
   * objects
   * then the exclusion strategies that were added will be applied as a
   * disjunction rule.
   * This means that if one of the added exclusion strategies suggests that a
   * field (or
   ...
   * {@link #setExclusionStrategies(ExclusionStrategy...)}
   ...
   * {@link #setPrettyPrinting(FormattingStyle)}
   ...
   * (for example newline and indent).
   * This option only affects JSON serialization. By default Gson produces compact
   * JSON output without any formatting.
   *
   * <p>
  ...
  /**
   * Configures Gson to allow JSON data which does not strictly comply with the
   * JSON specification.
   *
   * <p>
   * Note: Due to legacy reasons most methods of Gson are always lenient,
   * regardless of
   ...
   * provided. You can
   * call this method or {@link #setDateFormat(int)} multiple times, but only the
   * last invocation
   ...
   * class. See the documentation in {@link java.text.SimpleDateFormat} for more
   * information on
   ...
   * {@link java.text.DateFormat} for more
   ...
   * "{@code null}-safe". This
   * means when trying to serialize {@code null}, Gson will write a JSON
   * {@code null} and the
   * serializer is not called. Similarly when deserializing a JSON {@code null},
   * Gson will emit
   * {@code null} without calling the deserializer. If it is desired to handle
   * {@code null} values,
   ...
   * the type
   * adapter needs to be configured based on the type of the field being
   * processed. Gson
   * is designed to handle a large number of factories, so you should consider
   * registering
   ...
   * {@link JsonSerializer} and
   * a {@link JsonDeserializer}. If a type adapter was previously registered for
   * the specified
   * type hierarchy, it is overridden. If a type adapter is registered for a
   * specific type in
   * the type hierarchy, it will be invoked instead of the one registered for the
   * type hierarchy.
   *
   ...
   * {@link Float#NaN},
   * {@link Float#POSITIVE_INFINITY}, {@link Float#NEGATIVE_INFINITY}, or a double
   * value
   * {@link Double#NaN}, {@link Double#POSITIVE_INFINITY},
   * {@link Double#NEGATIVE_INFINITY}, it
   * will throw an {@link IllegalArgumentException}. This method provides a way to
   * override the
   * default behavior when you know that the JSON receiver will be able to handle
   * these special
   * values.
   *
   * @return a reference to this {@code GsonBuilder} object to fulfill the
   ...
   * Java
   * runtimes. For example Android does not provide {@code Unsafe}, or only with
   * limited
   * functionality. Additionally {@code Unsafe} creates instances without
   * executing any
   * constructor or initializer block, or performing initialization of field
   * values. This can
   * lead to surprising and difficult to debug errors.
   * Therefore, to get reliable behavior regardless of which runtime is used, and
   * to detect
   * classes which cannot be deserialized in an early stage of development, this
   * method allows
   ...
   * from using
   * reflection for the serialization and deserialization of certain classes. The
   * logic in
   ...
   * all classes for which no {@link TypeAdapter} has been registered, and for
   * which no
   ...
   * its
   * members and cache the result. It is not guaranteed that the filter will be
   * used again
   * every time a class or its members are accessed during serialization or
   * deserialization.
   *
